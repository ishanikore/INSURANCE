...
def clean_data(raw_bytes: bytes) -> pd.DataFrame:
    ...
    df["PI_AGE"] = df["PI_AGE"].fillna(df["PI_AGE"].median())
...
# Load from bundled CSV if present, else show uploader
if os.path.exists(CSV_PATH):
    with open(CSV_PATH, "rb") as f:
        df = clean_data(f.read())
else:
    st.markdown("### 📂 Upload your dataset to get started")
    uploaded = st.file_uploader(
        "Upload **Insurance__1_.csv** (or any compatible CSV)",
        type=["csv"],
    )
    if uploaded is None:
        st.info("👆 Please upload the insurance CSV file above. The full dashboard will load instantly after upload.")
        st.markdown("""
        **Required columns:**  
        `POLICY_NO, PI_NAME, PI_GENDER, SUM_ASSURED, ZONE, PAYMENT_MODE, EARLY_NON,  
        PI_OCCUPATION, MEDICAL_NONMED, PI_STATE, REASON_FOR_CLAIM, PI_AGE, PI_ANNUAL_INCOME, POLICY_STATUS`
        """)
        st.stop()
    df = clean_data(uploaded.read())
